...
from nerf._init_spade import Generator_audio as Generator_audio
import itertools
...
def main():
    ...
    if cfg.dataset.type.lower() == "expression":
        ...
        audio_data = audio_dataloader.AudioDataset(
            mode='train',
            cfg=cfg
        )
    ...
    start_epoch = 0
    # if True:
    #     checkpoint = "/root/logs/obama_se_fixed_bg_512_paper_model_random_choose/checkpoint00010.ckpt"
    ...
    for epoch in tqdm(range(start_epoch, total_epochs), desc="Training", unit="epoch"):
        ...
        for i in range(len(training_data)):
            ...
            G_opt.zero_grad()
            driving_data = torch.Tensor(audio_data[i][4]).to(device)
            fake_frame = G(frame, image, driving_data)
        ...
        if epoch % 2 == 0:
            ...
            for idx in range(cfg.texture_refine.val_num):
                ...
                training_image = training_image.unsqueeze(0)
                training_image = training_image.permute(0, 3, 1, 2)
                driving_data = torch.Tensor(audio_val_data[idx][4]).to(device)
                with torch.no_grad():
                    generated_photo = G(frame1, image1, driving_data)
                    if True:  # to avoid negative values
