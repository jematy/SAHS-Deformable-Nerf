...
class MaskCrossEntropyLoss(nn.Module):
    ...
    def forward(self, mask, input, target):
        ...
        num_unmasked_pixels = torch.count_nonzero(mask, dim=0)
        num_unmasked_pixels[torch.where(num_unmasked_pixels == 0)] = 1
        cross_entropy = -torch.sum(target * torch.log(input + 1e-10), dim=-1).unsqueeze(-1)
        no_masked_cross_entropy = torch.mean(cross_entropy)
        masked_cross_entropy = torch.sum(cross_entropy * mask, dim=0) / num_unmasked_pixels
        if self.weights is None:
            self.weights = torch.ones((mask.shape[-1]), device=mask.device)
        masked_cross_entropy_weights = self.weights * masked_cross_entropy
        return no_masked_cross_entropy, masked_cross_entropy, masked_cross_entropy_weights
...
class MaskMSELoss(nn.Module):
    ...
    def forward(self, mask, input, target):
        ...
        num_unmasked_pixels = torch.count_nonzero(mask, dim=0)
        num_unmasked_pixels[torch.where(num_unmasked_pixels == 0)] = 1
        diff = torch.sum(torch.square(input - target), dim=-1).unsqueeze(-1)
        no_masked_mse = torch.mean(diff)
        masked_mse = torch.sum(diff * mask, dim=0) / num_unmasked_pixels
        if self.weights is None:
            self.weights = torch.ones((mask.shape[-1]), device=mask.device)
        masked_mse_weights = self.weights * masked_mse
        return no_masked_mse, masked_mse, masked_mse_weights
...
def get_ray_bundle_by_mask(
    height: int, width: int, intrinsics , tform_cam2world: torch.Tensor, mask, center = [0.5,0.5]
):
    ...
    mask = mask[...,None].expand(width,height,3)
    mask_inv = 1-mask

    ray_directions = torch.sum(
        directions[..., None, :] * tform_cam2world[:3, :3], dim=-1
    )
    ray_directions = mask_inv * directions + mask * ray_directions

    origins = torch.Tensor([[[0,0,0]]]).to(ray_directions.device).expand(width,height,3)
    ray_origins = tform_cam2world[:3, -1].expand(ray_directions.shape)
    ray_origins = mask_inv * origins + mask * ray_origins
