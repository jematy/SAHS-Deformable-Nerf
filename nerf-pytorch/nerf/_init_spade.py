...
class Generator_audio(nn.Module):
    def __init__(self):
        super(Generator_audio, self).__init__()

        self.idencoder = IdEncoder()
        self.refine_network = RefineNetwork(64, 128, 256)
        self.AudioNet = AudioNet(76, 16)
    def forward(self, I_src, I_raw, driving_data):
        fid1, fid2, fid3 = self.idencoder(I_src)
        driving_data = self.AudioNet(driving_data.unsqueeze(0))
        expanded_vector = driving_data.unsqueeze(1)
        desired_shape = (1, 256, 64, 64)
        result_tensor = expanded_vector.repeat(*desired_shape)
        out = self.refine_network(I_raw, fid1, fid2, result_tensor)
        return out
