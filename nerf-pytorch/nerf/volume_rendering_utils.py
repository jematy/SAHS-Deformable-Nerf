...
def volume_render_radiance_field(
    ...
    if background_prior is not None:
        ...
        if background_prior.shape[1] > 4:
            seg = torch.softmax(radiance_field[:, :-1, 3:-1], dim=-1)
            rgb = torch.cat((rgb, seg), dim=-1)
        rgb = torch.cat((rgb, radiance_field[:, -1, :-1].unsqueeze(1)), dim=1)
    else:
    ...
    # sigma_a = torch.nn.functional.relu(radiance_field[..., 3] + radiance_field[..., 4] + noise)
